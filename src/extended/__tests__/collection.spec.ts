...
describe('ExtendedCollection', () => {
  ...
  describe('findOne', () => {
    ...
    it('should work with find options', done => {
      ...
      parentQuery.value.mockReturnValue(Observable.of([{ id: 'i1', x: 10 }, { id: 'i2', x: 20 }, { id: 'i3', x: 30 }]));
      childQuery.value.mockReturnValue(
        Observable.of([
          // { id: 'i1', y: 100 },
          { id: 'i2', y: 200 },
          { id: 'i3', y: 300 },
  ...
  describe('insert', () => {
    ...
    it('should return merged inserted documents', done => {
      const docs = [{ id: 'i1', x: 10, y: 20 }, { id: 'i2', x: 100, y: 200 }];
    ...
    it('should insert the parent sub documents in parent collection', done => {
      ...
      collection.insert(docs).subscribe(
        ...
        err => done.fail(err),
        () => {
          expect(parent.insert).toHaveBeenCalledWith([{ id: 'i1', x: 10 }, { id: 'i2', x: 100 }]);
          done();
        }
      );
    });
    ...
    it('should insert the child sub documents in child collection', done => {
      ...
      collection.insert(docs).subscribe(
        ...
        err => done.fail(err),
        () => {
          expect(child.insert).toHaveBeenCalledWith([{ id: 'i1', y: 20 }, { id: 'i2', y: 200 }]);
          done();
        }
      );
    });
