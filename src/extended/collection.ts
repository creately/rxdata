...
export class ExtendedCollection implements ICollection {
  ...
  public insert(docOrDocs: any): Observable<any[]> {
    ...
    return Observable.combineLatest(this.parent.insert(parentDocs), this.child.insert(childDocs)).map(sets =>
      this._mergeDocumentArrays(...sets)
    );
  }
  ...
  public remove(filter: any): Observable<any[]> {
    return Observable.combineLatest(this.parent.remove(filter), this.child.remove(filter)).map(sets =>
      this._mergeDocumentArrays(...sets)
    );
  }
  ...
  protected _pickSubDocument(filter: Function, doc: any): any {
    return Object.keys(doc)
      ...
      .reduce((result, key) => Object.assign(result, { [key]: doc[key] }), {});
  }
  ...
  protected _pickSubChanges(filter: Function, changes: any): any {
    return Object.keys(changes).reduce((result, operator) => {
