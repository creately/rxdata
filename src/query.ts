...
export class Query implements IQuery {
  ...
  public value(): Observable<any> {
    return this._options.values.map(docs => this._filterDocuments(docs)).distinctUntilChanged(createCompareFn());
  }
...
export class SingleDocQuery implements IQuery {
  ...
  /**
     ...
     */
  public value(): Observable<any> {
    return this._base.value().map(docs => docs[0]).distinctUntilChanged(createCompareFn());
  }
}
