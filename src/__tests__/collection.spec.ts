...
describe('Collection', () => {
  ...
  describe('find', () => {
    ...
    it('should work with find options', done => {
      collection
        ...
        .subscribe(
          doc => {
            expect(doc).toEqual([{ id: 'i2', x: 20 }, { id: 'i3', x: 30 }]);
            done();
          },
          err => done.fail(err)
        );
  ...
  describe('insert', () => {
    ...
    it('should insert a new document in collection', done => {
      collection.insert({ id: 'i1', x: 10 }).switchMap(() => collection.find({}).value().take(1)).subscribe(
        docs => {
          expect(docs).toEqual([{ id: 'i1', x: 10 }]);
          done();
        },
        err => done.fail(err)
      );
    ...
    it('should insert new documents in collection', done => {
      collection
        .insert([{ id: 'i1', x: 10 }, { id: 'i2', x: 20 }])
        .switchMap(() => collection.find({}).value().take(1))
        .subscribe(
          docs => {
            expect(docs).toEqual([{ id: 'i1', x: 10 }, { id: 'i2', x: 20 }]);
            done();
          },
          err => done.fail(err)
        );
    ...
    it('should not store getters in the document object', done => {
      ...
      collection.insert(doc).switchMap(() => collection.find({}).value().take(1)).subscribe(
        docs => {
          expect(docs).toEqual([{ id: 'i1', x: 10 }]);
          done();
        },
    ...
    it('should replace if a document already exists with id', done => {
      collection
        ...
        .subscribe(
          docs => {
            expect(docs).toEqual([{ id: 'i1', x: 20 }]);
            done();
          },
          err => done.fail(err)
        );
  ...
  describe('update', () => {
    ...
    it('should update matched document', done => {
      collection
        ...
        .subscribe(
          docs => {
            expect(docs).toEqual([{ id: 'i1', x: 100 }, { id: 'i2', x: 20 }]);
            done();
          },
          err => done.fail(err)
        );
  ...
  describe('remove', () => {
    ...
    it('should return updated document', done => {
      collection.insert({ id: 'i1', x: 10 }).switchMap(() => collection.remove({ id: 'i1' })).subscribe(
        docs => {
          expect(docs).toEqual([{ id: 'i1', x: 10 }]);
          done();
        },
