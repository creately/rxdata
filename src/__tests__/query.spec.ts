...
describe('Query', () => {
  describe('value', () => {
    ...
    it('should return matches on subscription', done => {
      ...
      query.value().take(1).subscribe(
        data => {
          expect(data).toEqual([{ _id: 'i2', type: 'b', value: 2 }, { _id: 'i4', type: 'b', value: 1 }]);
          done();
        },
        err => done.fail(err)
      );
    ...
    it('should sort values by a field', done => {
      ...
      query.value().take(1).subscribe(
        data => {
          expect(data).toEqual([{ _id: 'i4', type: 'b', value: 1 }, { _id: 'i2', type: 'b', value: 2 }]);
          done();
        },
        err => done.fail(err)
      );
    ...
    it('should limit number of results', done => {
      ...
      query.value().take(1).subscribe(
        data => {
          expect(data).toEqual([{ _id: 'i2', type: 'b', value: 2 }]);
          done();
        },
        err => done.fail(err)
      );
    ...
    it('should not emit duplicate results', done => {
      ...
      query.value().subscribe(data => received.push(data), err => done.fail(err));
    });
  });
});
...
describe('SingleDocQuery', () => {
  describe('value', () => {
    ...
    const matches = [{ _id: 'i1', type: 'a', value: 5 }, { _id: 'i2', type: 'b', value: 2 }];
